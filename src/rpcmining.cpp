...
Value checkkernel(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(-9, "Graphcoin is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(-10, "Graphcoin is downloading blocks...");
...
Value mine(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Graphcoin is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Graphcoin is downloading blocks...");
