...
CzGRPHWallet::CzGRPHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgrph seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGRPHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGRPHSeed_deprecated()) {
                    LogPrintf("%s: Updated zGRPH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgrph seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgrph wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGRPHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGRPHCount(nCountLastUsed);
    else if (!walletdb.ReadZGRPHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGRPHWallet::Lock()
{
    seedMaster = 0;
}

void CzGRPHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGRPHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGRPH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGRPHWallet::LoadMintPoolFromDB()
{
...
void CzGRPHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGRPHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGRPHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgrphTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGRPHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGRPH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgrphTracker which also adds to database
    pwalletMain->zgrphTracker->Add(dMint, true);
...
void CzGRPHWallet::SeedToZGRPH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGRPHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGRPHWallet::GenerateDeterministicZGRPH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGRPHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGRPHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
