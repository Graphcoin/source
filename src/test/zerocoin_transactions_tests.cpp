...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zgrphTracker = unique_ptr<CzGRPHTracker>(new CzGRPHTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZGRPH_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZGRPH_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
