...
#ifndef GRAPHCOIN_ZGRPHWALLET_H
#define GRAPHCOIN_ZGRPHWALLET_H
...
public:
    CzGRPHWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGRPH(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGRPH(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GRAPHCOIN_ZGRPHWALLET_H
