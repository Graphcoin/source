...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 GRPH at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGRPH send
enum ZerocoinSpendStatus {
    ZGRPH_SPEND_OKAY = 0,                            // No error
    ZGRPH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGRPH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGRPH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGRPH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGRPH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGRPH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGRPH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGRPH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGRPH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGRPH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGRPH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGRPH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGRPH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGRPH_SPENT_USED_ZGRPH = 14,                      // Coin has already been spend
    ZGRPH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGRPH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZGrphBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGRPHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzgrphAuto) const;
    ...
    */
    ...
    CzGRPHWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGRPHTracker> zgrphTracker;
    ...
    void setZWallet(CzGRPHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zgrphTracker = std::unique_ptr<CzGRPHTracker>(new CzGRPHTracker(strWalletFile));
    }

    CzGRPHWallet* getZWallet() { return zwalletMain; }
    ...
    void setZGrphAutoBackups(bool fEnabled)
    {
    ...
    /** zGRPH reset */
    boost::signals2::signal<void()> NotifyzGRPHReset;
