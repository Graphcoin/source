...
#include "ui_zgrphcontroldialog.h"
...
std::set<std::string> ZGrphControlDialog::setSelectedMints;
std::set<CMintMeta> ZGrphControlDialog::setMints;

ZGrphControlDialog::ZGrphControlDialog(QWidget *parent) :
	QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
	ui(new Ui::ZGrphControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGrphControlDialog::~ZGrphControlDialog()
{
    delete ui;
}

void ZGrphControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGrphControlDialog::updateList()
{
...
void ZGrphControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGrphControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGrph_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGrphControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGrphControlDialog::GetSelectedMints()
{
...
void ZGrphControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
