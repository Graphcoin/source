...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GRPH);
    ...
    if (!settings.contains("nAnonymizeGraphcoinAmount"))
        settings.setValue("nAnonymizeGraphcoinAmount", 1000);

    nAnonymizeGraphcoinAmount = settings.value("nAnonymizeGraphcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGraphcoinAmount"))
        SoftSetArg("-anonymizegraphcoinamount", settings.value("nAnonymizeGraphcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in graphcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGraphcoinAmount:
            return QVariant(nAnonymizeGraphcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGraphcoinAmount:
            nAnonymizeGraphcoinAmount = value.toInt();
            settings.setValue("nAnonymizeGraphcoinAmount", nAnonymizeGraphcoinAmount);
            emit anonymizeGraphcoinAmountChanged(nAnonymizeGraphcoinAmount);
            break;
        case CoinControlFeatures:
