...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered graphcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 GRPH.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal graphcoin:URI (graphcoin:XyZ...?message=example)
            {
...
static void NotifyzGRPHReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzGRPHReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzGRPHReset.connect(boost::bind(NotifyzGRPHReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzGRPHReset.disconnect(boost::bind(NotifyzGRPHReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zgrphTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
