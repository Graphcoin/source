...
// Copyright (c) 2017-2018 The Graphcoin developers
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGRPH ought to be enough for anybody." - Bill Gates, 2017
    ui->zGRPHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGRPHSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGRPHpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGRC_clicked()
{
    LogPrintf("on_pushButtonMintzGRC_clicked");
    if (!walletModel || !walletModel->getOptionsModel()){
        LogPrintf("no walletModel or getOptionsModel");
        return;
    }

    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGRPH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zGRPH, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGRPH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGRPH in ") +
...
void PrivacyDialog::on_pushButtonSpendzGRC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGRPH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zGRPH, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zGRPH
        sendzGRPH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGRPH
    sendzGRPH();
}

void PrivacyDialog::on_pushButtonZGrcControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZGrphControlDialog* zGRPHControl = new ZGrphControlDialog(this);
    zGRPHControl->setModel(walletModel);
    zGRPHControl->exec();
}

void PrivacyDialog::setZGrphControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGRPHSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGRPH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Graphcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGRPHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGRPH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GRPH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGRPH</b>";
    ...
    // use mints from zGRPH selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGrphControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZGrphControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGRPH require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zGRPH"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZGRPH_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGRPH require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zGRPH"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zGRPH was spent successfully update the addressbook with the label
    ...
    // Clear zgrph selector in case it was used
    ZGrphControlDialog::setSelectedMints.clear();
    ui->labelzGRPHSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGRPH Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GRPH, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGRPH Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zgrphTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGRPH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGRPH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGRPH "));
    ui->labelzGRPHAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zGRPH </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zGRPH </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzGRPHSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        // Mint zGRPH
        ui->pushButtonMintzGRC->setEnabled(false);
        ui->pushButtonMintzGRC->setToolTip(tr("zGRPH is currently disabled due to maintenance."));

        // Spend zGRPH
        ui->pushButtonSpendzGRC->setEnabled(false);
        ui->pushButtonSpendzGRC->setToolTip(tr("zGRPH is currently disabled due to maintenance."));
    }
    else {
        // Mint zGRPH
        ui->pushButtonMintzGRC->setEnabled(true);
        ui->pushButtonMintzGRC->setToolTip(tr("PrivacyDialog", "Enter an amount of GRPH to convert to zGRPH", 0));

        // Spend zGRPH
        ui->pushButtonSpendzGRC->setEnabled(true);
        ui->pushButtonSpendzGRC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}