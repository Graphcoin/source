...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGraphcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGraphcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeGraphcoinAmount = strAnonymizeGraphcoinAmount.remove(strAnonymizeGraphcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGraphcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGraphcoinAmount*COIN) nMaxToAnonymize = nAnonymizeGraphcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGraphcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGraphcoinAmount));
        strAnonymizeGraphcoinAmount = strAnonymizeGraphcoinAmount.remove(strAnonymizeGraphcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGraphcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeGraphcoinAmount == 0){
