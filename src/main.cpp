...
const string strMessageMagic = "Graphcoin Signed Message:\n";
...
int64_t GetRewardByHeight(int nHeight)
{
  ...
  if (thisYear < 13) {
    nReward = rewardByYear[thisYear] * COIN;
  } else {
    nReward = 1 * COIN;
  }
	return nReward;
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	int64_t nSubsidy;
	if (nHeight < 1000)
		nSubsidy = 5000 * COIN;  // premine = 999 blocks @ 5k coins ea + 500 blocks @ 10 coins = 5M
...
{
  int64_t currentTargetSpacing = fProofOfStake ? TARGET_SPACING : TARGET_SPACING_POW;
	CBigNum bnTargetLimit = fProofOfStake ? GetProofOfStakeLimit(pindexLast->nHeight) : Params().ProofOfWorkLimit();

	if (pindexLast == NULL){
	  if (fDebug) { LogPrintf("GetNextTargetRequired() : genesis block %d\n", bnTargetLimit.GetCompact()); }
	  return bnTargetLimit.GetCompact(); // genesis block
	}
	
	const CBlockIndex* pindexPrev = GetLastBlockIndex(pindexLast, fProofOfStake);
	if (pindexPrev->pprev == NULL){
	  if (fDebug) { LogPrintf("GetNextTargetRequired() : first block %d\n", bnTargetLimit.GetCompact()); }
	  return bnTargetLimit.GetCompact(); // first block
	}
	
	const CBlockIndex* pindexPrevPrev = GetLastBlockIndex(pindexPrev->pprev, fProofOfStake);
	if (pindexPrevPrev->pprev == NULL){
	  if (fDebug) { LogPrintf("GetNextTargetRequired() : second block %d\n", bnTargetLimit.GetCompact()); }
	...
	if (bnNew <= 0 || bnNew > bnTargetLimit){
	  if (fDebug) { LogPrintf("GetNextTargetRequired() : bnNew:%d bnTargetLimit:%d\n", bnNew, bnTargetLimit); }
	  bnNew = bnTargetLimit;
	}

	if (fDebug) {
	  LogPrintf("GetNextTargetRequired() : nInterval:%d nTargetTimespan:%d currentTargetSpacing:%d nAcutalSpacing:%d\n", nInterval, nTargetTimespan, currentTargetSpacing, nActualSpacing);
	  LogPrintf("GetNextTargetRequired() : prevBits:%d bnNew:%d\n", pindexPrev->nBits, bnNew.GetCompact());
	}
		...
		if (MasternodePayments)
		{
			...
			if (IsProofOfStake() && pindex != NULL) {
				if (pindex->GetBlockHash() == hashPrevBlock) {
					...
					CGraphcoinAddress address2(address1);
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
  ...
  if (!vtx[0].vout[0].IsEmpty()){
    if (fDebug) { LogPrintf("SignBlock() failed : !vtx[0].vout[0].IsEmpty()\n"); }
    return false;
  }
  ...
  if (IsProofOfStake()){
    if (fDebug) { LogPrintf("SignBlock() succeeded: isProofOfStake()\n"); }
    return true;
  }

  //static int64_t nLastCoinStakeSearchTime = GetAdjustedTime(); // startup timestamp
	if (fDebug) { LogPrintf("SignBlock() nLastCoinStakeSearchTime:%d time:%d timeOffset:%d\n", nLastCoinStakeSearchTime, GetTime(), GetTimeOffset()); }
	
	CKey key;
	CTransaction txCoinStake;
	if (fDebug) { LogPrintf("SignBlock() txCoinStake.nTime:%d\n", txCoinStake.nTime); }
	txCoinStake.nTime &= ~STAKE_TIMESTAMP_MASK;

	int64_t nSearchTime = txCoinStake.nTime; // search to current time
	if (fDebug) { LogPrintf("SignBlock() nSearchTime:%d\n", nSearchTime); }
		...
		if (wallet.CreateCoinStake(wallet, nBits, nSearchInterval, nFees, txCoinStake, key))
		{
			if (txCoinStake.nTime >= pindexBest->GetPastTimeLimit() + 1)
			{
				...
				for (vector<CTransaction>::iterator it = vtx.begin(); it != vtx.end();)
					...
					bool success = key.Sign(GetHash(), vchBlockSig);
					if (fDebug && !success) { LogPrintf("SignBlock() failed : key.Sign returned false\n"); }
					return success;
			} else {
			  if (fDebug) { LogPrintf("SignBlock() failed : txCoinStake.nTime(%d) >= pindexBest->GetPastTimeLimit()(%d)\n", txCoinStake.nTime, pindexBest->GetPastTimeLimit() + 1); }
			}
		} else {
		  if (fDebug) { LogPrintf("SignBlock() failed : wallet.CreateCoinStake returned false\n"); }
		}
		nLastCoinStakeSearchInterval = nSearchTime - nLastCoinStakeSearchTime;
		nLastCoinStakeSearchTime = nSearchTime;
	} else {
	  if (fDebug) { LogPrintf("SignBlock() failed : nSearchTime(%d) <= nLastCoinStakeSearchTime(%d)\n", nSearchTime, nLastCoinStakeSearchTime); }
	}
	return false;
}
#endif
