...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CGraphcoinAddress(redeemScript.GetID()).ToString();
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s GRPH %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough Darksend non-denominated funds for this transaction that are not equal 10000 GRPH.");
                    } else {
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CGraphcoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CGraphcoinAddress addrTo(ckidTo);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (nBalance <= nReserveBalance){
      if(fDebug){LogPrintf("CreateCoinStake failed nBalance <= nReserveBalance\n");}
      return false;
    }
    ...
    if (!SelectCoinsForStaking(nBalance - nReserveBalance, txNew.nTime, setCoins, nValueIn)){
        if(fDebug){LogPrintf("CreateCoinStake failed could not select coins for staking\n");}
        return false;
    }

    if (setCoins.empty()){
        if(fDebug){LogPrintf("CreateCoinStake failed setCoins is empty\n");}
        return false;
    }
    ...
    if (nCredit == 0 || nCredit > nBalance - nReserveBalance){
        if(fDebug){LogPrintf("CreateCoinStake failed no credit\n");}
        return false;
    }
    ...
    int64_t nReward;
    {
        ...
        if (!txNew.GetCoinAge(txdb, pindexPrev, nCoinAge)){
	     if(fDebug){LogPrintf("CreateCoinStake failed to calculate coin age\n");}
            return error("CreateCoinStake : failed to calculate coin age");
	}
	
        nReward = GetProofOfStakeReward(pindexPrev, nCoinAge, nFees);
        if (nReward <= 0){
	    if(fDebug){LogPrintf("CreateCoinStake failed no reward\n");}
            return false;
	}
    ...
    BOOST_FOREACH(const CWalletTx* pcoin, vwtxPrev)
    {
      if (!SignSignature(*this, *pcoin, txNew, nIn++)){
	    if(fDebug){LogPrintf("CreateCoinStake failed to sign coinstake\n");}
    ...
    if (nBytes >= MAX_BLOCK_SIZE_GEN/5){
              if(fDebug){LogPrintf("CreateCoinStake exceeded coinstake size limit\n");}
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CGraphcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CGraphcoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CGraphcoinAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s GRPH %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s GRPH %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
