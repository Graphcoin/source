...
public:
    ...
    bool WriteZGRPHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGRPHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGRPHSeed_deprecated(uint256& seed);
    bool EraseZGRPHSeed();
    bool EraseZGRPHSeed_deprecated();

    bool WriteZGRPHCount(const uint32_t& nCount);
    bool ReadZGRPHCount(uint32_t& nCount);
