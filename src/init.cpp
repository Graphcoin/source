...
void PrepareShutdown()
{
    ...
    RenameThread("graphcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "graphcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "graphcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GRPH and zGRPH money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-banscore=<n>", strprintf(_("Threshold for disconnecting misbehaving peers (default: %u)"), 10000));
    strUsage += HelpMessageOpt("-bantime=<n>", strprintf(_("Number of seconds to keep misbehaving peers from reconnecting (default: %u)"), 60));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 22629, 51484));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GRPH/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GRPH/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, graphcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GRPH/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Graphcoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-tupstake=<n>", strprintf(_("Enable or disable staking functionality for GRPH inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgrphstake=<n>", strprintf(_("Enable or disable staking functionality for zGRPH inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:22629"));
    ...
    strUsage += HelpMessageOpt("-backupzgrph=<n>", strprintf(_("Enable automatic wallet backups triggered after each zGRPH minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgrphbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zGRPH backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizegraphcoinamount=<n>     " + strprintf(_("Keep N GRPH anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Graphcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("graphcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Graphcoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Graphcoin Core is shutting down."));
...
#endif
    // Make sure only a single Graphcoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Graphcoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Graphcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //Graphcoin: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Graphcoin specific: zerocoin and spork DB's
                ...
                // Graphcoin: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Graphcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Graphcoin Core to complete") << "\n";
        ...
        zwalletMain = new CzGRPHWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zGRPHWallet
        uiInterface.InitMessage(_("Syncing zGRPH wallet..."));

        bool fEnableZGrphBackups = GetBoolArg("-backupzgrph", true);
        pwalletMain->setZGrphAutoBackups(fEnableZGrphBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zgrphTracker->Init();
...
//    }
//
//    nAnonymizeGraphcoinAmount = GetArg("-anonymizegraphcoinamount", 0);
//    if (nAnonymizeGraphcoinAmount > 999999) nAnonymizeGraphcoinAmount = 999999;
//    if (nAnonymizeGraphcoinAmount < 2) nAnonymizeGraphcoinAmount = 2;
    ...
    LogPrintf("Anonymize Graphcoin Amount %d\n", nAnonymizeGraphcoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1GRPH+1000 == (.1GRPH+100)*10
       10GRPH+10000 == (1GRPH+1000)*10
    */
